...
export const getLovedPlaces = (state: AppState): Place[] => state.places.loved;

export const isPlaceLoved = (placeId: string): (state: AppState) => boolean =>
    (state: AppState): boolean => !!getLovedPlaces(state).filter((place: Place) => place.slug === placeId).length;
